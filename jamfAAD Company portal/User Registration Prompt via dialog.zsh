...
####################################################################################################
#
# User Registration Prompt refer https://github.com/benwhitis/Jamf_Conditional_Access
#
####################################################################################################
####################################################################################################
#
...
jamfProPolicyID="${7:-"1154"}"                                  # Parameter 7: The Jamf Pro ID for the device compliance registration policy
  ...
  if [[ "$useSwiftDialog" == "true" ]]; then
    ...
    case ${returnCode} in 
      ...
      0) updateScriptLog "${scriptFunctionalName}: ${loggedInUser} clicked OK; "
        ...
        su - "${loggedInUser}" -c "/usr/bin/open \"jamfselfservice://content?entity=policy&id=$jamfProPolicyID&action=execute\""
      ;;
