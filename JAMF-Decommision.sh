...
# HISTORY
#
#   Version 1.9.0, 07-Apr-2023, Dan K. Snelson (@dan-snelson) & manikadnan
#
####################################################################################################
#sudo rm -rf /Library/Application\ Support/Dialog/Dialog.app
#sudo rm -rf /usr/local/bin/dialog
#sleep 1
####################################################################################################
#
# Global Variables 205,513-516,1235,1318,1320,1325-1327,1425-1430,1479-1487   2286
#
####################################################################################################
...
# Define the function to remove profiles
function remove_profiles {

        open /System/Library/PreferencePanes/Profiles.prefPane
        updateScriptLog "QUIT SCRIPT: Sheduled to removed the MDM profiles"
        eval "${jamfBinary} policy -trigger removeframework" 
}
...
title="Hi ${loggedInUserFirstname}, Revoking ABB-Owned Software"
...
bannerText="Hi ${loggedInUserFirstname}, Revoking ABB-Owned Software"
helpmessage="If you need assistance, please contact the ABB Global Service Department:  \n- **Email:** ABBGlobalServiceDesk@abb.com  \n\n**Computer Information:**  \n- **Operating System:**  ${macOSproductVersion} (${macOSbuildVersion})  \n- **Serial Number:** ${serialNumber}  \n- **Dialog:** ${dialogVersion}  \n- **Started:** ${timestamp}"
...
setupYourMacPolicyArrayIconPrefixUrl="https://your server url:8443/icon?id="
    ...
    case ${symConfiguration} in
        ...
        * ) # Catch-all (i.e., used when `welcomeDialog` is set to `video` or `false`)
            ...
            policyJSON='
            {
                "steps": [
                    {
                        "listitem": "Enterprise Connect",
                        "icon": "34",
                        "progresstext": "Uninstalling Enterprise Connect",
                        "trigger_list": [
                            {
                                "trigger": "uninstallenterprise",
                                "validation": "/Applications/Enterprise Connect.app/Contents/Info.plist"
                            }
                        ]
                    }
                ]
...
function finalise(){
    ...
    if [[ "${jamfProPolicyTriggerFailure}" == "failed" ]]; then
        ...
        dialogUpdateSetupYourMac "title: ${loggedInUserFirstname} your Mac is removed from ABB!"
    ...
    case ${validation} in
        ...
        */* ) 
            ...
            if [[ "${previouslyInstalled}" == "true" ]]; then
                dialogUpdateSetupYourMac "listitem: index: $i, status: success, statustext: App-not-exists"
            elif [[ -f "${validation}" ]]; then
                dialogUpdateSetupYourMac "listitem: index: $i, status: fail, statustext: Unable-to-Delete"
...
for (( i=0; i<dialog_step_length; i++ )); do 
    ...
    if [[ -n "$listitem" ]]; then
        ...
        dialogUpdateSetupYourMac "listitem: index: $i, status: wait, statustext: Validating…, "
    fi
