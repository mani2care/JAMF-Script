...
###############################################################################
###############################################################################
# Add A Mac To A Static Group - Prompt For Group And Serial.sh
###############################################################################
...
#
# The script will attempt to add the Mac to the selected group, then displays
...
pickTheGroup="choose from list every paragraph of \"$staticGroupList\" with title \"Select The Group To Modify\" with prompt \"Choose ONE group to add a Mac to...\" multiple selections allowed false empty selection allowed false"
#
...
gatherSerial="display dialog \"Enter target the Mac's serial number:\" default answer \"\" with title \"Define The Mac To Add\" buttons {\"Cancel\", \"OK\"} default button {\"OK\"}"
#
...
addComputer=$( curl \
	...
	-d "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>${apiDataAddToGroup}" $jssHost"JSSResource/computergroups/name/"$staticGroupName )
##############################################################################
##############################################################################
##############################################################################
#
...
if [ "$code" = "201 Request to create or update object successful" ]; 
	then
		displayResults="display dialog \"$serialNumber was added to the static group successfully\" with title \"Mac Successfully Added To Group\" buttons {\"OK\"} default button {\"OK\"}"
