...
# Write header to CSV
    echo "Device_ID,Computer,updateAction,versionType,specificVersion,maxDeferrals,forceInstallLocalDateTime,status,Error_Reason" > "$csvFilePath"
    
    # Extract data and append to CSV
    jq -r '.results[] | [.device.deviceId, .device.objectType, .updateAction, .versionType, .specificVersion, .maxDeferrals, .forceInstallLocalDateTime, .status.state, .status.errorReasons[0]] | @csv' "$jsonFilePath" >> "$csvFilePath"
